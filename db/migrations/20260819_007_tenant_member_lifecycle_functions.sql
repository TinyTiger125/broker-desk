-- TASK-039 Slice 1: owner-authorized member role and lifecycle writes.
-- Runtime connections cannot satisfy tenant_memberships UPDATE RLS directly;
-- these functions bind the actor to the current Clerk-backed local user and
-- preserve explicit capability presets rather than inferring permission from
-- legacy job roles.
