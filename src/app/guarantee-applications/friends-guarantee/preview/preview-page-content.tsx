...
type GuaranteeApplicationPreviewSearchParams = {
  caseId?: string;
  engine?: string;
  flash?: string;
  templateId?: string;
...
export type GuaranteeApplicationPreviewMode = "broker" | "authoring";

export type SharedGuaranteeApplicationPreviewPageProps = {
  mode?: GuaranteeApplicationPreviewMode;
  searchParams?: Promise<GuaranteeApplicationPreviewSearchParams>;
  templateId?: string;
};
...
export async function GuaranteeApplicationPreviewPage({
  searchParams,
  mode = "broker",
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-black tracking-tight text-slate-950">
              {isTemplateAuthoring
                ? `${template.companyDisplayName}公式テンプレートを校正する`
                : `${template.companyDisplayName}申込書を確認する`}
            </h1>
            <p className="mt-1 text-sm text-slate-600">
              {isTemplateAuthoring
                ? "公式底版、入力枠、フィールド紐付けはここでだけ管理します。保存内容は次の案件に反映されます。"
                : "安全項目は自動入力し、残りは申込書上で確認・修正してから印字します。"}
            </p>
            {templateNeedsCalibration ? (
              <p className="mt-2 inline-flex rounded-full bg-amber-100 px-3 py-1 text-xs font-bold text-amber-800">
                {isTemplateAuthoring
                  ? "校正中：保存前に枠位置と文字の収まりを確認してください"
                  : "この申込書はプレビュー確認が必要です。入力内容と印字位置を確認してから保存してください"}
              </p>
            ) : null}
          ...
          <div className="flex flex-wrap items-center gap-2">
            {isTemplateAuthoring ? (
              <Link href="/platform/templates" className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">
            ...
            {!isTemplateAuthoring && canDownloadPreview ? (
        ...
        <section className="min-h-[calc(100vh-132px)] rounded-xl border border-slate-200 bg-white shadow-sm">
          {selectedCase ? (
            <FriendsGuaranteeCalibrationPreview
              ...
              mode={isTemplateAuthoring ? "authoring" : "broker"}
            />
          ) : (
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className={`rounded-full px-2 py-1 text-[11px] font-bold ${isTemplateAuthoring ? "bg-violet-100 text-violet-800" : downloadGate?.canDownload ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-800"}`}>
                {isTemplateAuthoring ? "校正環境" : downloadGate?.canDownload ? "ダウンロード可" : `要確認 ${downloadGate?.blockedReasons.length ?? requiredMissingCount}`}
              </span>
            </div>
            {isTemplateAuthoring ? (
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                <Link href={companyDraftHref} className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-900 hover:bg-emerald-100">
                  申込書追加情報 {draftReadiness.readyCount}/{draftReadiness.fields.length}
                </Link>
                <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-bold text-slate-700">
                  追加情報保存 {formatDate(draft?.lastReviewedAt ?? draft?.updatedAt, "ja")}
                </div>
              </div>
            )}
            {templateNeedsCalibration ? (
              <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-900">
                {isTemplateAuthoring
                  ? "長い住所・分割マス・電話番号・金額欄の位置を確認し、必要なら入力枠、紐付け、標準位置を調整してください。"
                  : "データが全部入っていても、長い住所・分割マス・電話番号・金額欄はプレビューで確認してください。必要な場合は、この案件だけの印字位置を調整できます。"}
              </div>
            ) : null}
            ...
            <form id="guarantee-application-preview-form" action={saveGuaranteeApplicationTemplateCalibrationAction} className="space-y-4">
              ...
              <section className="rounded-xl border border-violet-200 bg-violet-50 p-4">
                ...
                <p className="mt-1 text-xs leading-5 text-violet-800">
                  ここで保存するのは、公式書類の入力枠・入力項目・標準位置です。案件データや申込書追加情報は変更されません。
                </p>
              </section>
...
export default async function BrokerGuaranteeApplicationPreviewPage({
  searchParams,
}: GuaranteeApplicationPreviewPageProps) {
  return GuaranteeApplicationPreviewPage({ searchParams, mode: "broker" });
}
