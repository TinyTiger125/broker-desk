...
export default async function GuaranteeFormEditPage({ params, searchParams }: { params: Promise<{ formId: string }>; searchParams?: Promise<{ blankFormVersionId?: string; maskId?: string }> }) {
  ...
  try {
    ...
    return <GuaranteeSlice1Client enabled={enabled} isAdmin={capabilityHasTenantPermission(getTenantCapability(session.membership), "template.edit_draft")} cases={testCases.map((item) => ({ id: item.id, title: item.caseTitle }))} publishedVersions={[]} initialMaskVersionId={selectedVersion?.id} initialBlankFormId={shouldLoadExplicitUpload || !selectedVersion ? form.id : undefined} initialBlankFormVersionId={shouldLoadExplicitUpload ? initialBlankFormVersionId : undefined} initialMaskId={shouldLoadExplicitUpload ? initialMaskId : undefined} adminOnly showUpload={false} heading={`编辑公司表格：${form.name}`} />;
  } catch (error) {
