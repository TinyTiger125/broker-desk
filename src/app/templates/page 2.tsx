...
import { listBrokerageCases, listOutputTemplateVersions, listTenantGuaranteeTemplateInstalls } from "@/lib/data";
...
const copyByLocale = {
  ja: {
    ...
    advancedDesc: "通常の申込書作成では開く必要はありません。書式調整、出力設定、入力位置の見直しに使います。",
    statusActive: "利用中",
    statusDraft: "下書き",
    statusDeprecated: "停止",
    verified: "検証済み",
    ...
    noCase: "案件がないため、先に案件を作成してください。",
    install: "ワークスペースに追加",
    installed: "追加済み",
    installDesc: "追加時点の公式版を独立コピーします。以後の公式更新でこのコピーは上書きされません。",
    installVersion: "元の公開版",
  },
  zh: {
    title: "申请书模板管理",
    ...
    advancedDesc: "普通申请书制作不需要打开。仅用于格式修订、输出设置和填写位置复核。",
    statusActive: "使用中",
    statusDraft: "草稿",
    statusDeprecated: "停用",
    verified: "已校验",
    ...
    install: "添加到工作区",
    installed: "已添加",
    installDesc: "添加后将生成当前工作区的独立副本；之后官方更新不会覆盖此副本。",
    installVersion: "来源版本",
  },
  ko: {
    title: "신청서 템플릿 관리",
    ...
    optional: "선택 항목",
    companyOnly: "회사별 항목",
    versionHistory: "수정 기록",
    noVersions: "수정 기록이 아직 없습니다.",
    qualityNotes: "품질 메모",
    noQualityNotes: "품질 메모가 없습니다.",
    advancedTitle: "상세 템플릿 관리",
    advancedDesc: "일반 신청서 작성에는 열 필요가 없습니다. 서식 수정, 출력 설정, 입력 위치 검토에 사용합니다.",
    statusActive: "사용 중",
    statusDraft: "초안",
    statusDeprecated: "중지",
    verified: "검증됨",
    ...
    noCase: "안건이 없으므로 먼저 안건을 만들어 주세요.",
    install: "워크스페이스에 추가",
    installed: "추가됨",
    installDesc: "추가 시점의 공식판을 독립 복사합니다. 이후 공식 업데이트가 이 복사본을 덮어쓰지 않습니다.",
    installVersion: "원본 게시판",
  },
} as const;
...
export default async function TemplatesPage({ searchParams }: TemplatesPageProps) {
  ...
  const [versions, cases, installedTemplates] = await Promise.all([
    ...
    <div className="space-y-5">
      ...
      <section className="grid min-w-0 gap-5 2xl:grid-cols-[minmax(17rem,20rem)_minmax(0,1fr)]">
        <aside className="self-start rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="space-y-2">
            {templates.map((template) => {
              ...
              const installed = installedByTemplateId.get(template.id);
              return (
                <Link
                  key={template.id}
                ...
                >
                  ...
                  {installed ? (
                    <p className="mt-2 text-[11px] font-bold text-emerald-700">
                      {copy.installed} / {copy.installVersion} v{installed.sourceVersionNumber}
                    </p>
                  ) : null}
                </Link>
              );
        ...
        <div className="min-w-0 space-y-4">
          ...
          <section className="flex flex-col justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:flex-row md:items-center">
            <p className="max-w-3xl text-sm leading-6 text-slate-600">{copy.installDesc}</p>
            {installedByTemplateId.has(selectedTemplate.id) ? (
              <span className="shrink-0 rounded bg-emerald-100 px-3 py-2 text-sm font-black text-emerald-800">
                {copy.installed} / {copy.installVersion} v{installedByTemplateId.get(selectedTemplate.id)?.sourceVersionNumber}
              </span>
            ) : (
              <form action={installGuaranteeTemplateForTenantAction}>
                <input type="hidden" name="templateId" value={selectedTemplate.id} />
                <button className="shrink-0 rounded bg-slate-950 px-4 py-2 text-sm font-black text-white hover:bg-slate-800">
                  {copy.install}
                </button>
              </form>
            )}
