...
export function GuaranteeApplicationClient({ enabled, caseId, caseTitle, caseFacts, publishedVersions, initialHistory, canGenerate }: Props) {
  ...
  const [guaranteeCompany, setGuaranteeCompany] = useState("friends_guarantee");
  const [applicationDate, setApplicationDate] = useState(() => new Date().toISOString().slice(0, 10));
  const [planType, setPlanType] = useState("住居用標準プラン");
  const [consent, setConsent] = useState(false);
  const [collectionAgency, setCollectionAgency] = useState("");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (typeof supplement?.["application.guarantee_company"] === "string") setGuaranteeCompany(String(supplement["application.guarantee_company"]));
        if (typeof supplement?.["application.application_date"] === "string" && supplement["application.application_date"]) setApplicationDate(String(supplement["application.application_date"]));
        if (typeof supplement?.["company_option.friends_plan_type"] === "string" && supplement["company_option.friends_plan_type"]) setPlanType(String(supplement["company_option.friends_plan_type"]));
        if (typeof supplement?.["company_option.friends_collection_agency"] === "string") setCollectionAgency(String(supplement["company_option.friends_collection_agency"]));
        if (typeof supplement?.["company_option.friends_single_rider"] === "string") setSingleRider(String(supplement["company_option.friends_single_rider"]));
        if (typeof supplement?.["company_option.friends_notes"] === "string") setNotes(String(supplement["company_option.friends_notes"]));
