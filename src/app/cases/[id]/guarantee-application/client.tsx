...
export function GuaranteeApplicationClient({ enabled, caseId, caseTitle, caseFacts, publishedVersions, initialHistory, canGenerate }: Props) {
  ...
  const openHistoryFile = async (item: HistoryItem) => {
    ...
    try {
      ...
      if (!response.ok || !contentType.includes("application/pdf")) {
        ...
        throw new Error(requestId ? `历史文件暂时无法打开，请重试。案件资料未受影响。请求编号：${requestId}` : "历史文件暂时无法打开，请重试。案件资料未受影响。");
      }
      const bytes = await response.blob();
      if (!bytes.size) throw new Error("历史文件为空，请重试。案件资料未受影响。");
      if (historyPreviewUrl) window.URL.revokeObjectURL(historyPreviewUrl);
      setHistoryPreviewUrl(window.URL.createObjectURL(bytes));
      setMessage("历史 PDF 已加载，可在当前案件页面查看。");
    } catch (caught) {
      setHistoryErrorId(item.id);
      setError(caught instanceof Error ? caught.message : "历史文件暂时无法打开，请重试。案件资料未受影响。");
    } finally {
      setHistoryLoadingId("");
    }
  };
  ...
  return <main className="mx-auto max-w-4xl px-6 py-10">
    ...
    <section className="mt-10 border-t border-slate-200 pt-8" aria-labelledby="history-title"><h2 id="history-title" className="text-lg font-semibold text-slate-950">案件申请书历史</h2>{history.length === 0 ? <p className="mt-3 text-sm text-slate-600">尚无已生成文件。</p> : <ul className="mt-4 divide-y divide-slate-200 border-y border-slate-200">{history.map((item) => <li key={item.id} className="flex flex-wrap items-center justify-between gap-3 py-3"><span className="text-sm text-slate-700">版本 v{item.version || "-"} · {new Date(item.generatedAt).toLocaleString("ja-JP")}</span>{item.fileReady ? <button type="button" className="text-sm text-blue-700 underline disabled:opacity-50" disabled={historyLoadingId === item.id} onClick={() => void openHistoryFile(item)}>{historyLoadingId === item.id ? "正在打开…" : historyErrorId === item.id ? "重试查看 PDF" : "查看 PDF"}</button> : <span className="text-sm text-slate-500">文件暂不可用</span>}</li>)}</ul>}{historyPreviewUrl && <iframe title="历史申请书 PDF" src={historyPreviewUrl} className="mt-6 h-[560px] w-full rounded border border-slate-200" />}</section>
  </main>;
}
