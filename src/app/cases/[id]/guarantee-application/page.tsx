...
import { getBrokerageCaseById, getGuaranteeCompanyMask, getGuaranteeMaskMatch, listGuaranteeBlankForms, listGuaranteeCompanyMaskVersions, listGuaranteeOutputsByCase } from "@/lib/data";
...
export default async function GuaranteeApplicationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let enabled = false;
  let brokerageCase: Awaited<ReturnType<typeof getBrokerageCaseById>> = null;
  ...
  try {
    ...
    brokerageCase = await getBrokerageCaseById({ userId: session.user.id, tenantId: session.tenant.id, caseId: id });
    if (!brokerageCase) {
      errorMessage = "案件不存在或当前身份无法访问";
    } else {
