  ...
  return (
    <div className="flex flex-wrap items-center gap-2" aria-busy={pending}>
      ...
      <label className="text-[11px] font-semibold text-slate-600">
        ...
        <input name="uploadFile" type="file" required disabled={pending} />
      </label>
      <button type="submit" disabled={pending} aria-disabled={pending} className="rounded-md border border-slate-300 px-2.5 py-1.5 text-xs font-bold text-slate-800 disabled:cursor-wait disabled:opacity-60">
        {pending ? text.processing : text.submit}
      </button>
      {pending ? <span role="status" aria-live="polite" className="text-[11px] font-semibold text-blue-700">{text.processing}</span> : null}
    </div>
  );
}
