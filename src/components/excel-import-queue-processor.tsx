  ...
  useEffect(() => {
    ...
    async function processJob() {
      try {
        ...
        if (!response.ok || !payload?.ok) throw new Error("excel_import_process_failed");
        if (cancelled) return;
        if (payload.status === "queued" || payload.status === "processing") {
          setStatus("queued");
          return;
        }
        if (payload.status === "failed") throw new Error("excel_import_process_failed");
        openReview();
      } catch {
  ...
  useEffect(() => {
    ...
    async function pollJob() {
      try {
        ...
        const payload = (await response.json().catch(() => null)) as ImportProcessResponse | null;
        if (!response.ok || !payload?.ok) throw new Error("excel_import_status_failed");
        if (cancelled || payload.status === "queued" || payload.status === "processing") return;
