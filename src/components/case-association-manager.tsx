  ...
  const renderObjectImport = (targetType: ObjectImportKind, targetId: string) => {
    ...
    return (
      <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3" data-object-import-target={`${targetType}:${targetId}`}>
        ...
        {objectImportUploadAction && !readOnly ? <div className="mt-3 border-t border-slate-200 pt-3"><ObjectImportUpload action={objectImportUploadAction} caseId={caseId} targetType={targetType} targetId={targetId} locale={locale} /></div> : null}
      </div>
    );
  };
