...
function getStatusLabel(locale: Locale, status: LocalReviewStatus) {
  const labels: Record<LocalReviewStatus, Record<Locale, string>> = {
    suggested: { ja: "確認待ち", zh: "待确认", ko: "확인 대기" },
    ...
    <form action={saveExtractionReviewAction} className="space-y-4">
      ...
      <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs leading-relaxed text-amber-900">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p>
            {tr(locale, {
              ja: "ここに表示している内容は確認待ちの読み取り結果です。保存すると採用・修正した項目だけが軽量案件の確認済みデータになります。正式な物件・顧客・見積へは自動登録されません。",
              zh: "这里显示的是待确认的读取结果。保存后，只有采用和修正的字段会进入轻量案件的已确认数据；不会自动写入正式物件、客户或报价。",
              ko: "여기에 표시된 내용은 확인 대기 중인 읽기 결과입니다. 저장하면 채택/수정한 항목만 경량 안건의 확인 데이터가 됩니다. 정식 매물, 고객, 견적에는 자동 등록되지 않습니다.",
            })}
          </p>
          <button
            type="button"
