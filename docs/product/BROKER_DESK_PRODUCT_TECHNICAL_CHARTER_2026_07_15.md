...
## 14. 2026-07-27 工程治理与 SaaS 上线审计基线

本章是下一阶段开发、测试、发布和真实数据使用的强制基线。产品路线仍以“资料输入 -> 事实核对 -> 可信输出”为主线，本次审计不改变产品方向，而是补齐把纵向原型演进为正式 SaaS 所必需的工程、数据和组织约束。

当本章与短期功能计划发生冲突时，优先处理本章定义的 P0 风险。任何发布节奏都不能替代质量门禁。
...
| --- | --- | --- |
| 本机开发与人工体验 | 可继续 | 模拟数据 |
| 朋友通过隧道进行受控测试 | 有条件可用 | 模拟数据或彻底匿名化数据 |
| 真实身份、合同、收入资料试点 | 暂不准入 | 不允许 |
| 付费客户试点 | 尚未就绪 | 不允许 |
| 公网正式 SaaS | 尚未就绪 | 不允许 |

当前产品已经证明核心工作流成立，也具备较好的确定性业务骨架，但仍属于“试点前纵向原型”。阻止真实上线的主要因素不是页面数量或 AI 功能数量，而是租户隔离、私有存储、身份认证、数据库恢复、发布纪律和云端可移植性尚未形成可验证闭环。

### 14.2 已确认的工程基础

本次审计确认以下基础可以保留并继续演进：

- `Next.js 16 + React 19 + TypeScript` 适合当前产品，不需要因上线目标重写前端框架。
- `PostgreSQL + Clerk` 适合作为多租户 SaaS 的数据与身份基础，但当前接入深度尚不足。
- TypeScript 严格检查、生产构建和现有安全测试可以通过。
- 当前实现已经具备权限判断、审计事件、CSV 注入防护、确定性字段提取、输出门禁和正式 PDF 生成等可复用能力。
- 产品已明确区分原始资料、读取候选、已确认事实和输出结果，这一事实链必须继续保持。
- “模型提出候选，人确认后成为事实，规则控制输出”仍是正确的 AI 边界。
...
- 生产构建通过。
- 现有生产安全和租户测试通过。
- 未完成真实 PostgreSQL、RLS、Clerk、私有对象存储和云端 OCR 的端到端验证。
- 未完成依赖漏洞联网审计；不能把“未发现”解释为“没有漏洞”。
- 为避免重置测试数据，本次未执行会重建或清空数据的全量回归脚本。

### 14.3 P0 上线阻塞项

以下问题未解决前，不得录入真实个人身份、合同、收入或权利资料：

1. **开发状态不可恢复**

   审计时开发工作区存在 62 个文件状态变化，其中包含大量已修改和未跟踪文件，变更规模约为 2,679 行新增、2,997 行删除。必须先形成可审查提交、远端备份和恢复点，禁止继续长期堆积在单一未提交工作区。

2. **代码仓库可见性不符合敏感业务要求**

   审计时 GitHub 仓库为公开状态。正式产品代码、部署配置、业务规则和未来客户资料处理逻辑必须按私有仓库标准管理，并启用分支保护、密钥扫描、依赖更新和最小权限。

3. **生产环境可能静默退回内存数据**

   当前数据层在未正确设置 `DATA_DRIVER=postgres` 或数据库连接时会使用内存实现。生产环境必须改为“配置缺失即启动失败”，不能以临时内存数据继续运行。

4. **租户与身份边界未形成端到端闭环**

   部分服务可以使用默认租户上下文，部分搜索接口缺少显式认证门禁。所有服务端入口必须从已验证身份解析租户，禁止客户端传入或服务端兜底为默认租户。

5. **数据库 RLS 不能作为当前安全证明**

   现有 RLS 策略尚未证明应用连接会设置租户上下文，数据库拥有者还可能绕过策略。必须使用非 owner 应用角色，在真实 PostgreSQL 上完成跨租户负向测试。

6. **上传资料不是私有云存储**

   当前默认路径可能把附件写入应用本地或公开目录。身份证、合同和申请资料必须进入私有对象存储，使用短时签名访问、租户前缀、服务端授权和可审计删除。

7. **资料读取依赖 macOS 本地能力**

   当前 OCR 路径包含 macOS/Swift 依赖，无法直接迁移到标准 Linux 云环境。必须抽象为可替换的读取服务，并在目标云环境验证日文证件、PDF 和 Excel。

8. **没有正式数据库迁移、备份与恢复体系**

   当前缺少可追踪 schema migration、恢复演练和明确 RPO/RTO。任何真实数据试点前必须完成迁移工具、每日备份、恢复演练和回滚文档。

9. **缺少生产级上传和后台任务保护**

   当前文件请求以内存处理为主，存在大文件、伪造扩展名、压缩炸弹、超时和重复提交风险。需要文件签名校验、大小限制、病毒扫描、任务队列、超时、幂等和失败重试。

10. **生产可观测性不足**

    必须建立结构化日志、错误追踪、核心流程指标、任务失败告警和审计日志查询。健康检查不得向外泄露原始数据库错误。

### 14.4 技术栈处置结论

不进行无证据的技术栈重写，按以下方式演进：

| 层 | 保留 | 必须补齐 |
| --- | --- | --- |
| Web | Next.js、React、TypeScript | 生产运行验证、错误边界、性能预算 |
| 身份 | Clerk | 开发/测试/生产独立实例，组织与租户强绑定 |
| 数据库 | PostgreSQL | 正式迁移、非 owner 连接、RLS 实测、连接池 |
| 文件 | 现有上传界面和资料模型 | 私有对象存储、签名访问、扫描、保留与删除策略 |
| 资料读取 | 现有确定性提取与候选值模型 | 云端 OCR 适配、异步队列、固定评测集 |
| AI | 候选、比较、建议和输出检查 | 数据最小化、超时、成本、审计、供应商治理 |
| 输出 | 保证公司 PDF 主链路 | 模板版本、输出快照、视觉回归、失败恢复 |
| 运维 | 当前本地服务脚本仅用于开发 | CI/CD、环境变量治理、监控、备份和回滚 |

通用输出中的占位实现不能被标记为“可生成”。除已经完成真实模板映射、门禁、预览和回归验证的保证公司申请书外，其他文书必须保持未开放状态。

### 14.5 产品体验审计结论

核心体验已经从“信息堆叠”向“对象选择 -> 分类核对 -> 单项处理”收敛，但仍有以下上线前风险：

- 整理信息首页的四类对象入口清楚，应继续保持首页与列表页分离。
- 案件、主体和物件编辑必须共享同一信息架构、状态语义和交互骨架，不能形成三套编辑器。
- 案件编辑页在常见桌面宽度下仍可能因超大断点才展开而退化为纵向堆叠，必须以 1280、1440 和折叠侧栏状态作为主要验收视口。
- 左侧章节不能无限增长；应使用强分类、二级分组、局部滚动或折叠，使当前值核对和编辑始终在首屏可达。
- 列表双击进入编辑效率高，但不可发现且不适合触屏。需要保留清楚的单击选中和键盘/触屏等价入口，不能只依赖双击。
- 工作台和录入资料的首要操作不能长期落在首屏以下。
- 必填项目设置的分类树和保存入口必须在常见视口内清楚可达。
- 对外界面继续删除内部语言、实现说明、重复动作和无业务意义的技术统计。
...
当前 A/B 双线的方向正确，但“两个工作区加两个端口”不等于正式环境隔离。

当前约定：

- B 线：持续开发环境，开发分支，本地端口 3002；周一、周三、周五可开放受控测试隧道。
- A 线：主测试环境，`main` 分支，本地端口 3001；只接收明确批准的合并。
- 周六：计划发布窗口，不是必须发布日。门禁未通过时必须取消发布。

正式 SaaS 必须演进为三个独立环境：

| 环境 | 用途 | 数据 | 发布来源 |
| --- | --- | --- | --- |
| Development | 日常开发与自动测试 | 模拟数据 | 功能分支或开发分支 |
| Staging | 验收、迁移演练、发布候选验证 | 模拟或批准的匿名化数据 | 固定 release candidate |
| Production | 正式客户服务 | 真实租户数据 | 已验收的不可变版本 |

三个环境必须分别拥有：

- 独立域名和部署资源。
- 独立 Clerk 实例或环境配置。
- 独立 PostgreSQL 数据库和数据库角色。
- 独立私有对象存储 bucket。
- 独立密钥、模型额度、日志和告警。
- 明确禁止开发环境直接访问生产数据。

主测试环境未来可以演进为 Production 的发布来源，但 Staging 和 Production 不得共享数据库、存储或运行进程。
...
需求与验收标准
  ...
  -> CI、代码审查和浏览器验收
  -> B 线受控体验
  -> release Pull Request：开发分支 -> main
  -> 数据迁移与备份检查
  -> A 线 / Staging 冒烟和用户验收
  -> 不可变版本标签
  -> Production 发布
...
- 不在 `main` 上直接开发。
- 不把未提交工作区当作唯一版本保存位置。
- 一个功能必须有需求、验收标准、风险说明和测试证据。
- 合并开发环境不等于自动上线生产。
- 数据迁移必须同时提供前滚、回滚或可恢复策略。
- 发布前先备份，发布后验证登录、上传、整理、输出和跨租户隔离。
- 重大事故修复走独立 hotfix，并回合并到开发分支。
- 周六只有在全部门禁通过时才发布，不能为了遵守日历牺牲稳定性。

### 14.8 CI/CD 最低门禁

Pull Request 合并前至少执行：
...
- 真实 PostgreSQL 集成测试。
- Clerk 登录、团队切换和越权测试。
- 私有对象存储上传、下载、删除和过期链接测试。
- 备份恢复演练。
- Linux 云环境 OCR/Excel/PDF 读取验证。
- 任务超时、重试、重复提交和断点恢复验证。

CI 通过只能证明自动检查通过，不能替代真实用户验收、数据迁移审查或发布负责人批准。

### 14.9 数据、隐私与 AI 治理

Broker Desk 将处理高敏感度个人与交易资料，必须以“默认不暴露、最少处理、可追溯删除”为原则。

最低要求：

- 建立数据分类：公开、内部、敏感、特敏。
- 对身份证件、收入、合同、联系方式和权利资料执行最小权限。
- 明确收集目的、保存期限、删除方式和导出方式。
- 日志、截图、错误报告和模型请求中不得记录不必要的原文或凭证。
- AI 请求只发送完成任务所需的最小字段或文档片段。
- 记录模型、版本、提示模板、输入摘要、输出摘要、成本和处理结果。
- 明确供应商的数据保留、训练使用、处理地域和再委托边界。
- 任何模型输出先进入候选或建议状态，不得绕过人工确认成为正式事实。
- 建立租户数据导出、账号关闭、资料删除和法律保留流程。
- 建立安全事件联系人、响应分级、通知和证据保全流程。

### 14.10 下一阶段强制实施顺序

下一阶段不得继续以页面微调为唯一主线，应按以下顺序推进：
...
1. 建立独立 Staging 资源和密钥。
2. 自动部署 release candidate。
3. 演练迁移、备份、恢复和回滚。
4. 在 Staging 完成完整纵向链路与跨租户验证。

#### 阶段 3：受控真实试点

1. 仅开放已经通过验证的输入类型和保证公司模板。
2. 以 1 至 2 家团队为范围，签署数据处理和试点边界。
3. 监控提取准确率、人工修正率、输出退回率和每案时间。
4. 在安全、恢复和退出流程验证后，才讨论付费与扩大租户。

### 14.11 不可降低的上线门禁

满足以下全部条件前，不得宣称“正式可上线”：

- 仓库、分支、密钥和发布记录可审计。
- Development、Staging、Production 真正隔离。
- 所有业务 API 经过身份验证和租户授权。
- 跨租户负向测试在真实数据库与存储上通过。
- 敏感附件使用私有对象存储，不可由公开路径访问。
- 正式数据库无内存回退，迁移和恢复演练通过。
- 关键后台任务有超时、幂等、重试和失败告警。
- 保证公司 PDF 在固定样本上通过视觉回归。
- 关键桌面视口、侧栏状态和键盘/触屏路径通过体验验收。
- 依赖漏洞、密钥扫描和隐私检查通过。
- 存在明确的发布负责人、回滚动作和事故响应流程。
...
- [Clerk Environment Management](https://clerk.com/docs/guides/development/managing-environments)
- [PostgreSQL Row Security Policies](https://www.postgresql.org/docs/17/ddl-rowsecurity.html)
- [GitHub Security and Analysis Settings](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/enabling-features-for-your-repository/managing-security-and-analysis-settings-for-your-repository?apiVersion=2022-11-28)
- [GitHub Protected Branches](https://docs.github.com/en/enterprise-server%403.18/repositories/configuring-branches-and-merges-in-your-repository/managing-protected-branches/about-protected-branches)
- [日本个人信息保护委员会通则指南](https://www.ppc.go.jp/personalinfo/legal/guidelines_tsusoku/)

### 14.13 2026-07-29 P0 受控处理状态

本节记录本次审计后在开发分支完成的“拒绝不安全运行”工作。它不表示生产准入完成，更不授权向 Production 录入真实资料。

| 审计问题 | 本次已处理的代码/流程边界 | 仍需完成的发布证据 |
| --- | --- | --- |
| 生产内存回退 | `NODE_ENV=production` 下未配置 Postgres、数据库连接或发布批准标识时，数据层拒绝运行；即使配置齐全，未完成请求级租户身份绑定也会拒绝运行。 | 独立 Staging/Production 数据库的实际运行记录。 |
| 生产认证 | Production 仅接受完整 Clerk 配置；开发可信请求头和演示登录在生产下失效。 | 独立 Clerk 实例、成员生命周期与真实访问验证。 |
| 迁移可追溯性 | 已建立 `000` 基线、`001` 租户 RLS、`002` 强制业务表 RLS 迁移，及带 advisory lock 的迁移账本执行脚本；Production 迁移须显式授权。 | 在 Staging 使用迁移角色执行、复核账本并演练回滚。 |
| 私有附件 | Development 附件改为服务端私有目录，通过租户授权下载；Production 拒绝本地目录和公开 HTTP URL，且在私有对象存储适配器缺失时强制拒绝，即使对象存储环境变量已配置。 | 私有对象存储上传、签名下载、删除适配器，文件签名/MIME 校验、恶意文件扫描与越权测试。 |
| 本机资料读取 | Production 拒绝 Swift 本机读取器，要求远程资料读取配置。 | Linux 可运行的异步读取服务、队列、超时、幂等、重试和失败恢复。 |
| RLS | 已提交 tenant RLS 迁移与策略基线，并对含 `tenant_id` 的业务表强制执行 RLS；授权查询表保持 RLS 启用，以避免 `SECURITY DEFINER` 身份解析产生递归策略校验；生产数据层会因请求级绑定尚未实现而拒绝启动。已提供只读 Staging 验证脚本以固定角色、事务身份和跨租户读隔离证据格式。 | **仍为阻断项：应用必须将当前 Clerk subject 在同一数据库事务内设置为 `app.external_auth_subject`，并以非 owner、无 `BYPASSRLS` 角色完成读写与跨租户并发负向测试。** |
| 运行与恢复 | 已形成生产准入运行手册。 | 备份恢复演练、可观测性、告警、事故响应和 GitHub 治理记录。 |

完整操作步骤与验收记录要求见 [P0 生产准入与恢复运行手册](../../operations/P0_PRODUCTION_READINESS_RUNBOOK_2026_07_29.md)。

在 RLS 请求事务上下文、私有对象存储、云端异步读取、恢复演练和观测告警全部完成之前，Production 数据运行必须保持拒绝状态，本项目只能作为 Development 或 Staging 体验版本运行。此约束优先于任何功能交付或试点时间表。

## 15. 分阶段路线图

本路线图必须服从第 14 章的工程治理与上线门禁。P0 安全、恢复和环境隔离未完成时，不得用新增功能替代基础建设。
...
从 2026-07-27 起，以上产品工程顺序必须与第 14.10 节的 SaaS 基础建设并行管理；任何涉及真实数据或对外发布的工作，必须先满足第 14.11 节门禁。
