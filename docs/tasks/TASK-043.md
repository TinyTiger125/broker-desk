...
- Preview 精确邀请验收确认 Clerk verified identity、subject→local user 绑定、local email 与 invitation 均一致，但 acceptance Action 先读取 verified identity 后又通过独立 `getDefaultUser()` auth/cache 路径解析本地用户，形成同一请求内两份身份快照并错误返回 `invitation_email_mismatch`。根修为把 Action 已验证的同一 identity 对象显式传入 data facade resolver，subject lookup、pending invitation bind 与非生产 fallback 均优先复用该对象；不放宽 email 或 subject 绑定。源码合同独立禁止恢复第二次 default-user 身份解析。修复后的真实 Preview 接受仍待复验。
