...
- `db/migrations/20260829_001_platform_owner_invited_email.sql`
...
- Preview 精确邀请验收经安全运行时证据确认 Clerk verified primary email、subject、DB-bound user 与 local email 全部一致；临时 harness 曾因共享文案先匹配而误分类为 `invitation_email_mismatch`，真实 Action token 为 `invitation_unavailable`。根因是 platform create 的初始 owner membership 未持久化 `invited_email`，而接受函数对 legacy NULL fail-closed。已在新 follow-up migration `20260829_001_platform_owner_invited_email.sql` 中承载 PostgreSQL runtime 修复，并以固定 SHA-256 合同保持已在 Staging ledger 应用的 `20260828_001_tenant_service_period.sql` 字节不变：新建 memory/PG owner 持久化规范化 invited email；prepare/refresh 在锁定 target membership/user 后补齐旧 NULL；accept 仅在 authenticated current user、tenant Tokyo service、pending membership、token与expiry全部通过后，以已绑定 target user 的规范化 email 在同一事务/clone 中回填并继续原子 acceptance audit。follow-up 的 legacy rename 有重复执行保护，legacy primitive 对 PUBLIC/runtime 均撤权，原签名 wrapper 恢复固定 search path、SECURITY DEFINER 与 runtime-only ACL；不要求人工重发且不放宽 subject/user/token/email 绑定。所有 `[TASK043_ACCEPT_DIAG]` 已移除；新 migration 尚未连接数据库或执行，修复后的真实 Preview 接受仍待复验。
