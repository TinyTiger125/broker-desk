# PROJECT_MEMORY 历史快照（2026-08-06）

> 来源：原 `docs/PROJECT_MEMORY.md`，基于 MIG-002 执行前正式仓库内容。
> 仅历史证据，不是当前任务、产品、架构或执行授权。
> 以下分隔线之后逐字保留原 `docs/PROJECT_MEMORY.md` 第 1-633 行；不得将本文件作为默认启动入口。

<!-- BEGIN ORIGINAL PROJECT_MEMORY.md (lines 1-633) -->
