...
async function expectLoginRejected(route, password, ca, phase, record) {
  ...
  try {
    ...
    record("login_rejected", { role: route.logicalRole, phase, sqlstate: safeSqlState(error) });
    return;
  }
  record("login_rejection_failed", { role: route.logicalRole, phase });
  throw new Error("login_rejection_failed");
}

async function cleanupRuntimeState({ client, input, ca, credentials, connections, record }) {
  try {
    for (const connection of connections.splice(0)) await connection.end().catch(() => undefined);
    record("test_connections_closed");
    for (const role of ROLES) {
    ...
    for (const role of ROLES) {
      ...
      if (password) await expectLoginRejected(input.roleRoutes[role], password, ca, "after_nologin", record);
    }
    const sessions = (await client.query("SELECT pid, usename, application_name FROM pg_stat_activity WHERE application_name = ANY($1::text[])", [Object.values(APP_NAMES)])).rows;
    ...
    if (final.roles.some((row) => row.rolcanlogin) || final.passwordState.some((row) => !row.password_is_null) || final.sessions.length !== 0) throw new Error("final_cleanup_state_mismatch");
    for (const role of ROLES) credentials.delete(role);
    record("credentials_released", { roles: ROLES });
    record("cleanup_completed", { finishedAtUtc: new Date().toISOString(), credentialsReleasedAfterRejectionProbe: true });
    return final;
  } catch (error) {
    record("cleanup_incomplete", { sqlstate: safeSqlState(error) });
    throw error;
  }
}
...
async function runCloud(input) {
  ...
  try {
    ...
    if (classification.phase !== "complete" || migrationState.ledgerRows.length !== 44) throw new Error("baseline_initialization_mismatch");
    ...
    for (const role of ROLES) {
      ...
      record("role_login_verified", { role, currentUser: identity.current_user, sessionUser: identity.session_user, database: identity.current_database });
    }
    await cleanupRuntimeState({ client: managementClient, input, ca, credentials, connections, record });
    cleanupCompleted = true;
    record("entry_pass", { cloudExecuted: true, finalState: "NOLOGIN/PASSWORD NULL" });
    return { target: { ref: TARGET.ref, database: TARGET.database, host: TARGET.host, port: TARGET.port, commit: TARGET.commit }, events, result: "pass" };
  } catch (error) {
    record("stopped", { category: "sql_or_client_error", sqlstate: safeSqlState(error) });
    return { target: { ref: TARGET.ref, database: TARGET.database, host: TARGET.host, port: TARGET.port, commit: TARGET.commit }, events, result: "stopped" };
  } finally {
    if (managementClient && !cleanupCompleted) {
      try {
        const ca = readAndVerifyCa(input.management);
        await cleanupRuntimeState({ client: managementClient, input, ca, credentials, connections, record });
      } catch (error) {
