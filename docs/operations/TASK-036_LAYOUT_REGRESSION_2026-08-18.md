...
- 本地不再继续账号、工作区或端口排查；剩余真实业务回归延期至预发布、Preview 或受控内部服务器环境。
- 环境必须绑定明确 Git SHA，使用一个内部测试身份和一个非生产 `active workspace`，不使用真实客户资料，并具备回滚到迁移前稳定版本的路径。
- 不把生产环境当测试环境，不要求第二账号或第二租户。
- 部署后只补 List Report 筛选/分页/返回、Responsive Form 非法值/错误焦点/合法保存/取消、一次真实 Kotoeri、Object Page、Worklist、Board、Relationship Explorer、首页和 Settings 既有保存。
- 57 张截图、22 路由冒烟和三宽度证据不重跑；仅对补测发现的 P0/P1 页面重跑。
- 只有差额全部通过、测试数据恢复或审计影响明确记录、无 Layout System 范围 P0/P1 且独立审查通过，才可改为 `Done`；当前保持 `Blocked`，输出专题未开始。
