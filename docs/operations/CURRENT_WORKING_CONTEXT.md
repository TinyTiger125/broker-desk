...
- `TASK-029` 当前为 `In Progress`：Checkpoint C 有限实现和独立只读审查已完成，代码提交 `8686d9a`，等待产品负责人复审，不自动进入 D-Lite。`/parties` 已移除 CSV checkbox、批量工具和导出按钮；现有 `/api/hub/export?scope=parties` 不修改，主体 CSV 因字段契约和空选择语义不可靠而冻结，未来恢复需独立任务；`flash` 只作创建/更新/归档/恢复后的短暂反馈，不是筛选、选中对象或业务状态。页面仅使用显式主体元数据，移除完成度/案件数量/输出推导、focus/第二详情和案件动作；主体名称为唯一主要进入链接，关系图与归档/恢复为次级操作。契约脚本、lint、typecheck、build、workflow 和 diff check 通过；product-language 的既有 `/import-center`“字段映射”失败与本任务无关。浏览器项目仍为 `UNVERIFIED`（localhost:3002 连接拒绝），`src/app/clients/page 2.tsx` 继续排除。
