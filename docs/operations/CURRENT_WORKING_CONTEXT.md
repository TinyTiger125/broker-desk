...
- `TASK-031 / W7-A` 当前为 `In Progress`，Checkpoint A 产品复审通过，已进入 Checkpoint B 目标结构规格：`docs/operations/TASK-031_W7A_TARGET_STRUCTURE_2026-08-18.md` 固定创建/编辑共同字段、术语、分组、名称必填、空值/零值/非法值、单一保存、白名单 `returnTo`、结构化错误与焦点、移除新建页 `FormDraftAssist`、移除完成度/进度工作台、lifecycle/关系边界、Responsive Form 三/二/一列和 PostgreSQL 最小读写适配。创建与编辑保留各自页面控制器和成功跳转，不强行合并两个表单或 Server Action。当前只修改治理文档，不启动服务或 Agent；TASK-020、输出专题、`/templates` W3、其他 W7/W8 页面和 `src/app/clients/page 2.tsx` 继续冻结/排除。真实浏览器、响应式、键盘、焦点、权限、租户和保存闭环仍未验证，需在后续实现/D-Lite或批次回归中如实标记。
