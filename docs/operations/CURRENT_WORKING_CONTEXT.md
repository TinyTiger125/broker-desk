...
- `TASK-029` 当前为 `In Progress`：Checkpoint C 最终结构微调提交 `d0cf288` 和独立只读审查已完成，等待产品负责人最终收口。页面标题已独立为 `pageTitle`，桌面结果区补齐名称/类型/角色/状态/操作表头，头部移除重复结果范围，无主体空态移除重复新增主体按钮；其余纯 List Report、显式主体元数据、CSV 页面冻结、flash 短暂反馈和范围边界不变。契约脚本、lint、typecheck、build、workflow 和 diff check 通过。D-Lite 为 `UNVERIFIED`：本地 demo 服务可监听 3002，但 `/parties` 因 Next.js `pg-connection-string` 无法解析 `fs` 的浏览器构建错误返回 500；服务已停止，未进入认证/租户排查。`src/app/clients/page 2.tsx` 继续排除。
