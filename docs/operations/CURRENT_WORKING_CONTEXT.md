...
- `TASK-025` 当前为 `Blocked`：Checkpoint A、B、C 已通过；代码提交 `4e80716` 已通过独立只读审查；Checkpoint D 预检已完成但未启动浏览器验收或独立审查。W1 当前只交付案件、主体、物件三个可靠对象类型；待归属资料入口、列表、数量和状态徽章因缺少具体对象归属权威而冻结，不代表资料为零或页面已完成。`targetEntity`、案件 `sourceImportJobIds` 和导入处理状态不足以证明具体主体/物件归属；未来恢复需独立产品和数据决策。阻塞原因仅为尚未确认两个可合法使用的非生产测试租户及其 Clerk 身份，以及没有可恢复的合成数据验证分页、长列表滚动和归档恢复。当前正式角色模型下所有角色均包含 `record.read`，已登录但无该权限的页面门禁为 `N/A`，不创建虚构角色。案件、主体、物件业务权威保持分离；列表不建立独立 selected 状态，分页使用 URL 的 `page`，浏览器返回恢复触发链接焦点，一级导航继续返回对象选择器并重置列表上下文。状态契约不授权新建业务完成算法，不显示或推导输出资格。TASK-020 继续 Blocked，输出中心和最终预览/下载延期为独立输出产品专题，首页最后处理。
- `TASK-026` 当前为 `Blocked`：已通过正式平台账户管理流程创建 `TASK025 QA A` 和 `TASK025 QA B` 两个专用非生产租户，并发送 Clerk 开发环境邀请；QA B 已绑定，QA A 等待邀请接受，尚未创建业务测试数据。任务仍只覆盖两个专用测试租户、合法 Clerk 身份、现有角色、同一对象类型至少 7 条合成记录、正常路径归档/恢复、跨租户隔离和脱敏交接；不得修改业务代码、数据库结构、生产数据、认证规则、权限模型或租户隔离。两个身份均可登录后，才恢复 TASK-025 Checkpoint D。
