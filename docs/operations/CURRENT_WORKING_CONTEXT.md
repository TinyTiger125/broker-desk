...
- `TASK-038 / G1-SLICE-1` 当前为 `In Progress / Staging Acceptance Infrastructure — 429 Investigation`；公司表格库、人工校准、测试确认、发布、案件申请持久化、重复生成幂等和 v2/v3 版本基线已通过。当前 Durable Private Files 保持待验收，不关闭、不重新实现；B-1 申请页单次低频验收曾返回 Vercel 边缘 429，当前只诊断 429 来源并收口验收工具请求预算，停止重复访问。耐久文件、权限对照和异常 PDF 补验待限流窗口恢复后继续。
