...
- `TASK-027` 当前为 `Blocked`（Checkpoint A 环境阻塞）：已启动本地开发服务，但 `/clients` 被重定向到 `/sign-in?reason=login_required`，页面明确显示“登录服务尚未配置”，未取得合法登录会话，因此没有任何 `/clients` 业务运行结论。已提交只读阻塞报告 `docs/operations/evidence/TASK-027/2026-08-17/checkpoint-a-report.md`。恢复后只补做真实登录审计，不制作目标图、不修改 `src/`、不创建或修改客户数据、不启动实现或独立审查 Agent。客户与主体保持不同业务对象；阶段、用途、温度和跟进排序不套用案件/主体/物件状态。`/properties` 本轮只作为明确不选的反例，不审计、不迁移。TASK-020、输出专题和共享平台 QA 缺口继续独立保留。
