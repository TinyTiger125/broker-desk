...
- `TASK-029` 当前为 `In Progress`：Checkpoint A 只读审计完成，Checkpoint B 目标结构规格已提交至 `docs/operations/TASK-029_W6B_TARGET_STRUCTURE_2026-08-17.md`，等待产品负责人复审，不进入实现。规格收束为单一标准 List Report：继续使用 Client 兼容持久化但不宣称客户等于主体；类型/角色只认显式主体元数据；移除完成度、quotation/contract 案件数量、focus/默认第一条、右侧第二详情、输出前检查和案件动作；主体名称为唯一主要进入链接，关系图与归档/恢复为次级操作；CSV 只允许导出选中主体且未选择时禁用。浏览器项目仍为 `UNVERIFIED`（localhost:3002 连接拒绝）；未修改 `src/`、未启动 Agent、未触碰 `src/app/clients/page 2.tsx`。
