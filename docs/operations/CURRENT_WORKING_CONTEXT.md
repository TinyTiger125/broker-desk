...
- `TASK-038 / G1-SLICE-1` 当前为 `In Progress / Staging Product Validation — Durable Private Files & Access`；Vercel项目 Firewall 仅匹配 Staging Preview 主机的 `broker-desk-staging-preview-v1` 规则已从 100 次/IP/60 秒调整为 300 次/IP/60 秒，低频单会话复核未再出现429。B登录、工作区、案件列表和B-1详情均正常；点击“生成申请书”未跳转且未产生申请页业务请求，Durable Private Files尚未开始，需先处理该首个产品阻断。Production、认证、代码和数据库未改；回退方式仍为在同一规则页恢复100次/60秒并重新Publish。
