...
- `TASK-023 / UI-GOV-003 Checkpoint A` 已完成两张 1440×900 桌面目标图、一张 390×844 窄屏图、无横向溢出检查和顺序独立审查，状态为 `In Review`。两种模式共用案件外壳和 UI Foundation，布局保持不同；不得修改 `/cases/[id]`、修复 TASK-020、迁移其他页面或进入 Checkpoint B，当前等待产品负责人确认。
