...
- `TASK-038 / G1-SLICE-1` 当前为 `In Progress / Staging Product Validation — Durable Private Files & Access`；Staging 版本 `8267ca5` 已将案件详情的“申込书を生成”恢复为当前案件绑定的原生链接。独立隔离浏览器证据显示：B-1/B-2 的鼠标点击、B-1 键盘 Enter、直接地址均进入对应申请页；B 案件列表只显示 B-1/B-2；A 案件列表只显示 A-1，A 直接访问 B-1 得到“页面未找到”且不泄露案件资料；低频访问未出现 429。Durable Private Files、跨部署同字节下载和原始空白 PDF 权限仍待验收。Production、认证、数据库未改。
