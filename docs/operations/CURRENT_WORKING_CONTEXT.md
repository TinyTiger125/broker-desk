...
- `TASK-038 / G1-SLICE-1` 当前为 `In Progress / Runtime Defect Fix — Durable Private Files`；公司表格库、人工校准、测试确认、发布、案件申请持久化、重复生成幂等和 v2/v3 版本基线已通过。新 P1：B 能看到案件历史输出记录，但点击同一输出地址在 Staging 不稳定/返回不可用，重试曾回到首页；Vercel 运行日志对应输出路由出现 404。代码核对确认私有附件在 Postgres 路径写入 `private_attachment_blobs`，但未显式配置 `DATA_DRIVER` 时会回退 memory，不能证明跨实例耐久性。本轮已改为 Preview/Staging 在有数据库地址时使用 Postgres 私有 blob，历史读取校验 SHA/大小/MIME，缺失字节标记 `unavailable`，案件页改为当前页加载/失败/重试；新版本尚未部署，跨部署一致性仍待真实验证。异常 PDF 上传暂停。
