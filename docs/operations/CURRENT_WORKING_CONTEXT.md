...
- `TASK-030` 当前为 `In Progress`：W6-C `/properties` Checkpoint C 有限实现和独立只读审查已完成，无 P0/P1 阻塞。memory/PostgreSQL 的 `listQuoteFormData().properties` 已返回真实 `area`，`listHubProperties` 已移除名称推断；页面使用“区域/エリア”术语，费用通过原始 nullable 适配值保留 null/零值区别，listing price 非正数显示“未设置”，lifecycle 只使用 active/archived。实现严格限于批准文件，并对照 `/clients`、`/parties`、Layout System V1、`src/components/ui-foundation/` 和 `globals.css`，不新增 Token 或全站重构。固定趋势、完整度、关系、活动、随机封面、focus/侧栏、输出和 CSV 继续冻结。一次 D-Lite 运行探测因 `npm run dev -- --port 3002` 返回 `listen EPERM` 未能启动服务；响应式、键盘、空态、错误、返回、权限/租户和 CSV 下载均为 `UNVERIFIED`，未继续认证或环境排查。
