...
- `TASK-029` 已建立为 `Proposed`：W6-B 主体索引 `/parties` 只进入 Checkpoint A 只读审计，不进入目标设计或实现。审计必须先核对主体是否实际复用 `clients`、关联案件数量是否为合同/提案数量、完成度和角色/类型是否为非权威推断、`focus`/右侧面板是否构成第二套详情页，以及 CSV/归档/输出入口边界。不得修改 `src/`，不得启动 Agent，不得触碰 `src/app/clients/page 2.tsx`；浏览器当前 localhost:3002 连接拒绝，运行项目按 `UNVERIFIED` 处理。
