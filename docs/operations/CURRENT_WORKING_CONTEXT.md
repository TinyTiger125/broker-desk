...
- `TASK-029` 当前为 `In Progress`：Checkpoint A 只读审计完成，等待产品负责人复审，不进入目标设计或实现。审计确认 `/parties` 实际复用 `clients`，关联案件数使用 quotation/contract 计数，完成度由姓名/联系方式/角色/关联物件四项自算，`focus`/默认第一条驱动承载编辑、关系、输出和进度的右侧第二套主体详情；CSV 空选择会导出全部主体，且主体到 `/output-center` 使用的 `partyId` 不在目标页参数契约中。浏览器项目为 `UNVERIFIED`（localhost:3002 连接拒绝）；未修改 `src/`、未启动 Agent、未触碰 `src/app/clients/page 2.tsx`。Checkpoint B 前需由产品负责人决定主体权威对象、关联关系口径、完成度去留、详情层级、CSV 权限/空选择和输出入口边界。
