...
- `TASK-025` 继续 `In Progress`：Checkpoint A、B 已通过；当前只进行 Checkpoint C 前置准备，代码实现必须等待两份前置治理文档差异完成独立归属并形成明确干净基线。案件、主体、物件业务权威保持分离；列表不建立独立 selected 状态，分页使用 URL 的 `page`，浏览器返回恢复触发链接焦点，一级导航继续返回对象选择器并重置列表上下文。状态契约不授权新建业务完成算法，不显示或推导输出资格。TASK-020 继续 Blocked，输出中心和最终预览/下载延期为独立输出产品专题，首页最后处理。本轮未修改 `src/`、未启动 Agent。
