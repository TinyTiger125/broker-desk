...
- `TASK-031 / W7-A` 当前为 `In Progress`，Checkpoint A/B 产品复审已通过；Checkpoint C 已完成一次有限实现和独立只读审查（无 P0/P1）。实现共享 Responsive Form 字段、服务端解析/校验、严格 `returnTo`、错误摘要固定焦点、PostgreSQL `getPropertyById`/`updateProperty` 租户作用域、移除新建页 `FormDraftAssist` 和编辑完成度/工作台/sticky 保存。售价留空使用兼容 `0`，显式 `0` 拒绝，管理费/修缮费真实 `0` 保留；创建审计 `targetType` 为 `property`。契约脚本、TASK-030 契约脚本、lint、typecheck、build、workflow rules、diff check 已通过。D-Lite 仅探测一次本地服务，`npm run dev -- --port 3002` 返回 `listen EPERM`，未启动服务；真实浏览器、1440/768/390、横向溢出、Tab/Enter/返回焦点、PostgreSQL 运行读写、权限/租户、完整无障碍和 D-Lite 均标记 `UNVERIFIED`/进入批次回归；页头返回链接与底部取消入口同源为非阻塞观察。TASK-020、输出专题、`/templates` W3、其他 W7/W8 页面和 `src/app/clients/page 2.tsx` 继续冻结/排除。
