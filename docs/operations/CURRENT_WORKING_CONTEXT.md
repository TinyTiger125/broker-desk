...
- `TASK-022 / UI-GOV-002A 最小视觉基础` 已 Done；其基础组件已被 TASK-023 正式案件页参考实现使用。非导航开发预览路由已随 TASK-023 收口移除；不启动 UI-GOV-002B。
...
- `TASK-024 / UI-GOV-002B` 已建立并处于 `Ready`：目标是建立唯一 Broker Desk Layout System 和全路由 Floorplan 映射。当前只允许完成 Layout Contract 与目标图；目标图批准前不修改 `src/`，批准后才建立公共组合组件，并以案件总览“申请人”章节作为 Responsive Form 试点。禁止全站换皮、批量 CSS、第二套 Token 和批量页面迁移；首页最后处理。
