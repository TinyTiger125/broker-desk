...
- `TASK-026` 当前为 `Blocked`：正式 Clerk 开发登录和平台账户管理入口已验证可达，但两个专用非生产租户、合法 Clerk 测试身份及所有权尚未确认；尚未创建租户、身份或测试数据。任务仍只覆盖两个专用测试租户、合法 Clerk 身份、现有角色、同一对象类型至少 7 条合成记录、正常路径归档/恢复、跨租户隔离和脱敏交接；不得修改业务代码、数据库结构、生产数据、认证规则、权限模型或租户隔离。取得受控非生产 QA 身份和租户所有权后，才恢复 TASK-025 Checkpoint D。
