...
- `TASK-029` 当前为 `In Progress`：Checkpoint B 条件通过，目标规格已修正 CSV 与 `flash` 边界并授权进入 Checkpoint C。`/parties` 页面移除 CSV checkbox、批量工具和导出按钮；现有 `/api/hub/export?scope=parties` 不修改，主体 CSV 因字段契约和空选择语义不可靠而冻结，未来恢复需独立任务；`flash` 只作创建/更新/归档/恢复后的短暂反馈，不是筛选、选中对象或业务状态。其余单一 List Report 约束继续有效：显式主体元数据、无完成度/案件数量/输出推导、无 focus/第二详情、主体名称为唯一主要进入链接，关系图与归档/恢复为次级操作。下一步仅执行一次有限实现、一次独立只读审查和静态验证，不自动进入 D-Lite。浏览器项目仍为 `UNVERIFIED`（localhost:3002 连接拒绝）；`src/app/clients/page 2.tsx` 继续排除。
