...
- 当前批准的普通业务任务是 `TASK-020 实施 C+ 案件总览`：先完成案件总览生产实现、浏览器行为和独立审查；本轮只建立任务卡，尚未启动实现。`TASK-019 外部演示环境准备` 仍保持 Ready，负责真实 Clerk、专用测试账号、虚假测试数据、双租户隔离、隧道回调、安全边界和关闭/回退方案；完成前不得开放产品隧道。
