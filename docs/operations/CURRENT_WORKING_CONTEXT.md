...
- `TASK-040 / W9` 当前为 `In Progress`；W9.1已 `Accepted / Runtime Verified`，W9.2案件页面读取/写入接入已在精确 Preview `b239ce24873f5062d72be330f20b1708d1e24138` 取得产品验收并以 `88428cd` 合入 main，案件页面阶段状态为 `Product Accepted / Integrated`；人物页面读取/写入接入已在精确 Preview `9ea54fc6d2aad46140dd44fc844cc283a6e8ec34` 取得产品验收并以 `90dd52b8` 合入 main，人物页面阶段状态为 `Product Accepted / Integrated`。resolver、company_read 只读页面、案件/人物写 Action owner_write 门、申请书入口保护及关联对象 fail-closed 均已覆盖。物件页面、关系图、附件、PDF 来源权限、定向分享、共同编辑和资料接管仍未开始；跨经营主体浏览器＋写 Action 组合证据作为最终跨对象回归 P2 保留；Production不连接、不迁移。
...
- TASK-039 为 `Foundation Accepted and Integrated / Visibility Follow-up Pending`；案件、人物、物件可见范围及后续资料授权继续未完成。
- 平台候选已合入 main；`vercel.json` 保持 `git.deploymentEnabled.main=false`，不得部署 Production 或执行 Production migration。
