...
- `TASK-023 / UI-GOV-003` Checkpoint A 目标图已获产品负责人批准；Checkpoint B 正式 `/cases/[id]` 已实施并保持快速补全的任务导向、案件总览的对象导向。Checkpoint C 授权最小修复已完成：两个开发预览路由移除，`case-overview.tsx` 加入手动滚动 hash 同步并保持原生锚点语义。独立审查 Agent 一次性复验仅确认路由移除通过，仍缺少修复后手动滚动/hash、键盘锚点和隔离模板下载确认/修改失效的独立浏览器闭环证据；TASK-023 保持 `In Review`，不启动 UI-GOV-002B；TASK-020 仍独立 `Blocked`。
