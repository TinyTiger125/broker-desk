...
- `TASK-023 / UI-GOV-003` Checkpoint A 目标图已获产品负责人批准；Checkpoint B 正式 `/cases/[id]` 已实施并保持快速补全的任务导向、案件总览的对象导向。Checkpoint C 本地收口部分通过：模式切换、编辑焦点恢复、锚点点击/高亮、390/768 响应式和输出中心入口有证据；手动滚动未同步已有 hash、键盘锚点未证明、当前 demo 无可用模板无法完成最终下载确认/修改失效，且两个开发预览路由未受会话保护。TASK-023 保持 `In Review`，不启动 UI-GOV-002B；TASK-020 仍独立 `Blocked`，真实 Clerk/第二租户/生产服务只作为发布环境门禁记录。
