...
- `TASK-038 / G1-SLICE-1` 当前为 `In Progress / Staging Infrastructure Fix — Vercel Edge 429`；Vercel项目 Firewall 仅匹配 Staging Preview 主机的 `broker-desk-staging-preview-v1` 规则在 100 次/IP/60 秒时触发边缘 429，应用运行日志无对应业务请求。已在项目级、Preview-only 范围将阈值调整为 300 次/IP/60 秒并发布；Production、认证、代码和数据库未改。回退方式：在同一规则页恢复 100/60 秒并重新 Publish。待一次低频B回归后再恢复 Durable Private Files 验收。
