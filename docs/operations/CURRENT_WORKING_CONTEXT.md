...
- `TASK-030` 当前为 `In Progress`：W6-C `/properties` Checkpoint B 方向已批准，规格修正见 `docs/operations/TASK-030_W6C_TARGET_STRUCTURE_2026-08-18.md`。Checkpoint C 先在 memory/PostgreSQL 的 `listQuoteFormData().properties` 列表映射补回真实 `area`，再由 `listHubProperties` 移除名称推断；页面使用“区域/エリア”术语，费用保留 null/零值区别，listing price 非正数显示“未设置”，lifecycle 只使用 active/archived。实现严格限于批准文件，并对照 `/clients`、`/parties`、Layout System V1、`src/components/ui-foundation/` 和 `globals.css`，不新增 Token 或全站重构。固定趋势、完整度、关系、活动、随机封面、focus/侧栏、输出和 CSV 继续冻结。当前无 3000/3002 服务和合法浏览器证据，响应式、键盘、空态、错误、返回、权限/租户和 CSV 下载均为 `UNVERIFIED`；实现后只启动一次独立只读审查，再进入 D-Lite。
