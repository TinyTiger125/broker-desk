...
- `TASK-023 / UI-GOV-003 Checkpoint A` 桌面方向和 390×844 窄屏收口已完成；最新桌面/窄屏截图、静态门禁和最终独立复核通过，状态为 `In Review`，等待产品确认。只改预览作用域；不得修改正式 `/cases/[id]`、AppNav、TASK-020、其他页面或进入 Checkpoint B。生产 `npm start` 503 仍属于后续演示环境问题，不得与本原型证据混淆。
