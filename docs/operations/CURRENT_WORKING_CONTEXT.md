...
- `TASK-038 / G1-SLICE-1` 当前为 `In Progress / Runtime Defect Fix — Tenant Context Continuity`；公司表格库、人工校准、测试确认、发布、案件申请持久化、重复生成幂等和 v2/v3 版本基线已通过。只读核对确认 B-1/B-2 仍在同一 Neon 非生产主分支，数据和成员关系未删除或切换；当前 P1 是无有效当前经营主体时 `/organize-center` 进入 `tenant_selection_required` 全局错误态，页面没有引导用户选择公司。已补工作区选择提示、返回原页面和安全返回路径；10MB文件限制已在本地版本统一，尚待同一版本Staging真实验证。耐久文件、异常PDF和旧行为补验继续暂停。
