...
- `TASK-027` 当前为 `Proposed`：下一阶段只审计真实登录非生产环境中的 `/clients`，不制作目标图、不修改 `src/`、不创建或修改客户数据、不启动实现或独立审查 Agent。审计必须覆盖初始状态、默认排序/筛选、关键字、阶段/用途/温度筛选、排序组合、客户详情返回、快速创建取消、可安全复现的空/加载/错误/权限状态、1440/768/390、键盘和窄屏表格可理解性。客户与主体保持不同业务对象；阶段、用途、温度和跟进排序不套用案件/主体/物件状态。`/properties` 本轮只作为明确不选的反例，不审计、不迁移。TASK-020、输出专题和共享平台 QA 缺口继续独立保留。
