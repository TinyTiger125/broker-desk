...
- 日期：2026-08-18
- 状态：`In Progress` / Checkpoint C 有限实现完成，独立只读审查通过；D-Lite 运行证据待批次回归
- 前置：Checkpoint A 产品复审通过
- 目标：创建与编辑共享物件字段、术语、分组、校验和错误语言，但保留各自页面控制器、初始值和成功跳转
- 规格修正：售价显式 `0` 返回字段错误；保存失败焦点固定进入顶部错误摘要。实现已限于本规格允许文件；未启动第二轮视觉优化。
...
- 售价 `listingPrice`：可选；留空时沿用当前持久化兼容值 `0`，编辑页以空值/“未设置”显示；用户明确输入 `0` 必须返回字段错误，填写时必须是大于 `0` 的有效数字。负数和非法文本同样返回字段错误；不宣称数据库能够区分留空形成的兼容 `0` 与历史零值。
...
- 保存失败后焦点固定进入页面顶部错误摘要；摘要是可编程聚焦元素并使用 `role="alert"`，每条错误链接到对应字段；用户激活错误链接后焦点进入对应输入；
...
3. 售价留空保存为兼容 `0` 并在编辑页显示空值/“未设置”；显式输入 `0` 返回字段错误，不写入；
