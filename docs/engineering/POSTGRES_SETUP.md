...
Local development has one convenience fallback: a configured PlatformOwner without tenant membership can use the seeded default tenant as a temporary `platform_owner` tenant session so navigation across product modules does not crash during Clerk bootstrap. The fallback is enabled by default only outside production runtime. When testing a production build locally through `next start`, enable it explicitly:
