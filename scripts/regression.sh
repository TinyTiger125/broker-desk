...
echo "$dash_html" | grep '業務進捗' >/dev/null || fail "home page missing workflow progress section"
echo "$dash_html" | grep '未処理' >/dev/null || fail "home page missing pending queue"
echo "$dash_html" | grep 'よく使う入口' >/dev/null || fail "home page missing quick actions"
echo "$dash_html" | grep '申込書ステータス' >/dev/null || fail "home page missing application status section"
...
echo "$case_workbench_html" | grep '候補' >/dev/null || fail "case workbench missing candidate status label"
...
echo "$extractor_workbench_html" | grep '資料候補' >/dev/null || fail "extractor-key case missing field-level source candidate summary"
