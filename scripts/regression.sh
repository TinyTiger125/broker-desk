...
echo "$default_home_html" | grep '資料管理センター' >/dev/null || fail "default home page should be Japanese without locale cookie"
...
echo "$dash_html" | grep '案件を作る' >/dev/null || fail "home page missing create case entry"
echo "$dash_html" | grep '資料を読む' >/dev/null || fail "home page missing read material entry"
echo "$dash_html" | grep '足りない情報を入れる' >/dev/null || fail "home page missing organize entry"
echo "$dash_html" | grep '書類を出力' >/dev/null || fail "home page missing output entry"
echo "$dash_html" | grep '次に処理するもの' >/dev/null || fail "home page missing work queue"
echo "$dash_html" | grep '全体の残り' >/dev/null || fail "home page missing remaining overview"
...
echo "$output_html" | grep '書類を出力' >/dev/null || fail "output-center missing output center heading"
...
echo "$output_html" | grep '先に対象案件を選択' >/dev/null || fail "output-center missing case selector"
echo "$output_html" | grep '対象案件' >/dev/null || fail "output-center missing case selection label"
echo "$output_html" | grep '補助機能を表示' >/dev/null || fail "output-center missing secondary output disclosure"
echo "$output_html" | grep '１全保連.pdf' >/dev/null && fail "output-center exposes template source file name in company selector"
friends_html="$(curl -fsS "${BASE_URL}/output-center?caseId=${friends_fixture_case_id}&guaranteeTemplate=friends_guarantee_individual_v1")" || fail "friends guarantee output-center unreachable"
...
echo "$case_workbench_html" | grep 'この案件に資料を追加' >/dev/null || fail "case workbench missing case-scoped intake"
echo "$case_workbench_html" | grep '完成度と入力結果' >/dev/null || fail "case workbench missing progress and input summary"
echo "$case_workbench_html" | grep '今見ている入力結果' >/dev/null || fail "case workbench missing current input summary"
echo "$case_workbench_html" | grep 'id="case-main-editor"' >/dev/null || fail "case workbench missing main editor anchor"
echo "$case_workbench_html" | grep '入力ファイル・出典を表示' >/dev/null || fail "case workbench missing source evidence disclosure"
echo "$case_workbench_html" | grep '書類を出力' >/dev/null || fail "case workbench missing output handoff"
echo "$case_workbench_html" | grep 'この案件で使う項目' >/dev/null || fail "case workbench missing case applicability settings"
echo "$case_workbench_html" | grep 'name="condition:employment_required"' >/dev/null || fail "case workbench missing employment applicability control"
echo "$case_workbench_html" | grep '必須完了' >/dev/null || fail "case workbench missing required-complete progress wording"
...
echo "$case_property_node_html" | grep '完成度と入力結果' >/dev/null || fail "case workbench node missing progress and input summary"
echo "$case_property_node_html" | grep 'bg-indigo-50 text-indigo-950' >/dev/null || fail "case workbench node selection missing selected state"
echo "$case_property_node_html" | grep 'この分類に未処理の項目はありません' >/dev/null || fail "case workbench node should hide complete optional fields"
echo "$case_property_node_html" | grep 'name="field:property.usage"' >/dev/null && fail "case workbench node should not expose complete optional property field"
...
echo "$extractor_workbench_html" | grep '完成度と入力結果' >/dev/null || fail "extractor-key case missing progress and input summary"
echo "$extractor_workbench_html" | grep 'この案件で使う項目' >/dev/null || fail "extractor-key case missing case applicability settings"
echo "$extractor_workbench_html" | grep 'name="condition:employment_required"' >/dev/null || fail "extractor-key case missing employment applicability control"
echo "$extractor_workbench_html" | grep '必須完了' >/dev/null || fail "extractor-key case missing required-complete progress wording"
echo "$extractor_workbench_html" | grep 'name="field:property.postalCode"' >/dev/null || fail "extractor-key case missing unresolved property field"
echo "$extractor_workbench_html" | grep 'name="status:property.postalCode"' >/dev/null || fail "extractor-key case missing field status selector"
echo "$extractor_workbench_html" | grep 'name="field:applicant.employerPhone"' >/dev/null || fail "extractor-key case missing typed applicant phone control"
echo "$extractor_workbench_html" | grep 'type="tel"' >/dev/null || fail "extractor-key case phone field should render as tel"
echo "$extractor_workbench_html" | grep 'name="field:applicant.birthDate"' >/dev/null || fail "extractor-key case missing typed applicant birth date control"
...
echo "$extractor_workbench_html" | grep 'name="field:emergencyContact.relationship"' >/dev/null || fail "extractor-key case missing unresolved select field"
echo "$extractor_workbench_html" | grep '確認済みにする' >/dev/null || fail "extractor-key case missing confirmed decision label"
echo "$extractor_workbench_html" | grep '確認できない' >/dev/null || fail "extractor-key case missing unknown decision label"
echo "$extractor_workbench_html" | grep '使わない' >/dev/null || fail "extractor-key case missing rejected decision label"
echo "$extractor_workbench_html" | grep '保存して左に反映' >/dev/null || fail "extractor-key case missing field save action"
echo "$extractor_workbench_html" | grep 'この案件に資料を追加' >/dev/null || fail "extractor-key case missing compact case intake"
...
echo "$selected_friends_html" | grep '申込書追加情報' >/dev/null || fail "friends guarantee output missing application extra-info source label"
...
echo "$selected_friends_html" | grep "/guarantee-applications/friends_guarantee_individual_v1/preview?caseId=${friends_fixture_case_id}#company-draft-fields" >/dev/null || fail "friends guarantee output missing company-draft preview deep link"
echo "$selected_friends_html" | grep "/cases/${friends_fixture_case_id}?guaranteeTemplate=friends_guarantee_individual_v1&amp;node=" >/dev/null || fail "friends guarantee output missing template-scoped workbench node deep link"
...
echo "$quotes_html" | grep '/quotes/quote_yamada_a' >/dev/null || fail "quotes page missing stable quote fixture"
quote_path="/quotes/quote_yamada_a"
...
echo "$ai_experience_html" | grep '入力サポート設定' >/dev/null || fail "input support settings missing heading"
echo "$ai_experience_html" | grep '新しい候補を確認' >/dev/null || fail "input support settings missing update action"
