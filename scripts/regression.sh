...
echo "$seed_json" | grep '"brokerageCases":7' >/dev/null || fail "QA business data seed missing brokerage cases"
...
echo "$dash_html" | grep '資料マップ' >/dev/null || fail "home page missing material map"
echo "$dash_html" | grep '案件' >/dev/null || fail "home page missing case column"
echo "$dash_html" | grep '顧客/関係者' >/dev/null || fail "home page missing party column"
echo "$dash_html" | grep '物件' >/dev/null || fail "home page missing property column"
echo "$dash_html" | grep '資料ファイル' >/dev/null || fail "home page missing source file column"
echo "$dash_html" | grep '資料索引' >/dev/null || fail "home page missing object index"
echo "$dash_html" | grep '選択中' >/dev/null || fail "home page missing selected object panel"
echo "$dash_html" | grep '最近の更新' >/dev/null || fail "home page missing recent updates"
...
echo "$case_workbench_html" | grep '案件資料ツリー' >/dev/null || fail "case workbench missing dossier tree"
echo "$case_workbench_html" | grep '資料の地図' >/dev/null || fail "case workbench missing dossier map"
...
echo "$case_workbench_html" | grep '要確認' >/dev/null || fail "case workbench missing candidate status label"
...
case_workbench_all_html="$(curl -fsS "${BASE_URL}/cases/${friends_fixture_case_id}")" || fail "case workbench all-fields page unreachable"
...
echo "$case_property_node_html" | grep '現在の分類' >/dev/null || fail "case workbench node selection missing current category table"
...
echo "$case_property_node_html" | grep 'case-field-property-usage' >/dev/null || fail "case workbench node missing field-level anchor"
echo "$case_property_node_html" | grep 'name="field:property.usage"' >/dev/null || fail "case workbench node deep link missing unresolved property field"
echo "$case_workbench_all_html" | grep 'name="field:property.usage"' >/dev/null || fail "case workbench missing unresolved property field"
echo "$case_workbench_all_html" | grep 'name="field:applicant.homePhone"' >/dev/null || fail "case workbench missing unresolved applicant phone field"
echo "$case_workbench_all_html" | grep 'name="field:coOccupants.0.gender"' >/dev/null || fail "case workbench missing unresolved select field"
echo "$case_workbench_all_html" | grep 'name="field:applicant.homePhone"' >/dev/null || fail "case workbench missing typed applicant phone control"
...
echo "$case_workbench_all_html" | grep 'name="status:property.usage"' >/dev/null || fail "case workbench missing field decision selector"
echo "$case_workbench_all_html" | grep '確認済みにする' >/dev/null || fail "case workbench missing confirmed decision label"
echo "$case_workbench_all_html" | grep '確認できない' >/dev/null || fail "case workbench missing unknown decision label"
echo "$case_workbench_all_html" | grep '使わない' >/dev/null || fail "case workbench missing rejected decision label"
...
echo "$extractor_workbench_html" | grep '案件資料ツリー' >/dev/null || fail "extractor-key case missing dossier map"
...
echo "$extractor_workbench_html" | grep 'この項目を保存' >/dev/null || fail "extractor-key case missing one-field save action"
...
echo "$ai_experience_html" | grep '入力ルールの確認' >/dev/null || fail "input rule review missing heading"
echo "$ai_experience_html" | grep '提案を作成' >/dev/null || fail "input rule review missing proposal action"
