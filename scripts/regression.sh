...
curl() {
  command curl -H "Cookie: brokerdesk_locale=${REGRESSION_LOCALE}" "$@"
}
...
echo "[STEP] default Chinese locale smoke"
default_home_html="$(command curl -fsS "${BASE_URL}/")" || fail "default home page unreachable"
echo "$default_home_html" | grep '资料管理中心' >/dev/null || fail "default home page should be Chinese without locale cookie"
echo "$default_home_html" | grep '搜索客户、物件、案件、资料、输出文件' >/dev/null || fail "default Chinese home page missing global search"
...
echo "$dash_html" | grep '現在の案件進捗' >/dev/null || fail "home page missing case progress section"
echo "$dash_html" | grep '現在の詰まり' >/dev/null || fail "home page missing current blocker"
echo "$dash_html" | grep '種類で見る' >/dev/null || fail "home page missing object type navigation"
echo "$dash_html" | grep '資料索引' >/dev/null || fail "home page missing object index"
echo "$dash_html" | grep '紐付いた資料' >/dev/null || fail "home page missing linked source context"
...
echo "$case_property_node_html" | grep 'bg-indigo-100 text-indigo-900' >/dev/null || fail "case workbench node selection missing selected state"
echo "$case_property_node_html" | grep 'name="field:property.name"' >/dev/null || fail "case workbench node deep link missing property field"
...
echo "$extractor_workbench_html" | grep '読取結果' >/dev/null || fail "extractor-key case missing field-level source candidate summary"
...
echo "$extractor_workbench_html" | grep '表示中の項目をまとめて確認' >/dev/null || fail "extractor-key case missing visible candidate batch confirmation action"
...
echo "$friends_preview_html" | grep 'id="company-draft-fields"' >/dev/null || fail "friends guarantee preview missing company draft section anchor"
