...
mustInclude(formsClient, ["UPLOAD_ERROR_MESSAGES", "请选择PDF文件。", "无法读取该PDF。请确认文件未损坏后重新上传。", "暂不支持加密或密码保护的PDF。请解除保护后重新上传空白PDF。", "PDF文件不能超过10MB。", "第一版仅支持单页PDF。", "暂不支持带页面旋转设置的PDF。请导出为方向固定的单页PDF后重试。", "该PDF的页面裁切设置暂不支持。请重新导出为标准单页PDF后重试。", "请求编号", "explainUploadError"], "upload error UX contract");
...
mustInclude(formsEditPage, ["requireTenantSession", "template.edit_draft", "GuaranteeSlice1Client", "listBrokerageCases", "initialMaskVersionId", "initialBlankFormVersionId", "initialMaskId", "requestedVersion", "recoveryBlankFormVersionId", "recoveryMaskId", "selectedVersion", "no matching saved version exists", "loadInitialAdminContext", "readPrivateAttachmentContentForTenant", "blankPagePngBase64", "表格恢复失败，请稍后重试", "randomUUID", "if (!enabled)", "既有申请书路径不受影响", "activeVersionId !== blankVersion.id", "const initialAdminContext = selectedVersion", "const clientBlankFormId = selectedVersion || (recoveryBlankFormVersionId && recoveryMaskId) ? form.id : undefined"], "formal editor access contract");
