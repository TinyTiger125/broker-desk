...
- TASK-027 is In Progress. Checkpoint A runtime audit was constrained by the shared login/workspace environment and remains historical evidence, not a UX pass. The approved target is now the `/clients` List Report: find a customer and decide who to follow up next, while preserving customer/party separation and existing stage, purpose, temperature, follow-up, detail-entry, and create/save semantics. Checkpoint B/C may modify only `/clients`, customer-specific presentation components, and approved List Report/UI Foundation components; it must not modify auth, permissions, tenancy, database, customer domain logic, `/properties`, TASK-020, or output pages. Shared platform regression items are not page-level gates for this task.
