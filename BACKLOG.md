...
| TASK-030 | P0 | W6-C：物件索引 /properties List Report 参考迁移 | In Progress | TASK-029 | [TASK-030](docs/tasks/TASK-030.md) | Checkpoint A 只读代码审计完成；固定比例趋势、资料/输出准备度、静态活动、循环封面、字符串关系、focus/侧栏第二详情和空选择导出全部被登记为非权威或越界；浏览器、响应式、键盘、错误/空态、权限/租户运行证据 UNVERIFIED，等待产品负责人复审 |
