...
| TASK-018 | P0 | MIG-007恢复唯一可运行产品基线 | In Progress | TASK-002, TASK-017 | [TASK-018](docs/tasks/TASK-018.md) | A implementation/review and static checks pass; A browser/data gate is blocked by unavailable demo workspace access; B has not started |
...
- TASK-018 / MIG-007已进入实施：A 的 TASK-003 实现和独立审查已通过，静态检查通过；真实浏览器/数据行为门禁因无可用 demo 工作区身份和 in-app Browser 无法连接临时端口而阻塞，B 不得开始。TASK-005/006/007/008 继续暂缓。149 项专业分类已登记为 A/B 之外的朋友测试验收缺口。
- 开发服务可启动且公开数据健康检查为 `200 ready`，但受保护路由仍为登录重定向；`npm start` 的四个探测路由均为 `503`，首个明确阻断为生产边缘限流启用标志和策略 ID 缺失，其他生产就绪门禁也未满足。真实登录、外部数据库发布状态和隧道行为仍需人工验证。safety/WIP 与 WIP 快照保持冻结。
