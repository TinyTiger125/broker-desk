...
| TASK-027 | P0 | W2 客户索引 List Report 参考迁移 | Proposed | TASK-025 | [TASK-027](docs/tasks/TASK-027.md) | 仅 Checkpoint A 真实登录只读审计；不得制作目标图、修改页面或启动 Agent |
...
- TASK-027 is Proposed. It is limited to a real-login, read-only Checkpoint A audit of `/clients` as the second List Report candidate. It must preserve the distinction between customer and party semantics, must not write customer data, must not make target designs or code changes, and must not start implementation or review Agents. `/properties` remains an explicit out-of-scope comparison candidate for this round because its dashboard, selected detail, batch export, archive, relationship, and output-preparation structures are not yet product-split.
