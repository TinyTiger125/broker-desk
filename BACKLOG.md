...
| TASK-027 | P0 | W2 客户索引 List Report 参考迁移 | Blocked | TASK-025 | [TASK-027](docs/tasks/TASK-027.md) | Checkpoint A 因开发环境未配置登录服务而阻塞；尚未取得任何 /clients 业务运行证据，不得制作目标图、修改页面或启动 Agent |
...
- TASK-027 is Blocked at Checkpoint A because the current local development environment redirects `/clients` to `/sign-in?reason=login_required` and renders `登录服务尚未配置`; no legitimate logged-in `/clients` evidence was obtained. The task remains limited to a real-login, read-only audit, preserves customer/party semantics, writes no customer data, and must not make target designs or code changes or start Agents. `/properties` remains an explicit out-of-scope comparison candidate for this round because its dashboard, selected detail, batch export, archive, relationship, and output-preparation structures are not yet product-split.
