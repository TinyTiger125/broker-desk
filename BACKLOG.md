...
| TASK-030 | P0 | W6-C：物件索引 /properties List Report 参考迁移 | In Progress | TASK-029 | [TASK-030](docs/tasks/TASK-030.md) | Checkpoint A 产品复审通过；Checkpoint B 目标结构规格已提交，明确 lifecycle 具有 repository migration contract 但数据库实际应用未验证，updatedAt/趋势/完整度/关系/活动/随机封面/CSV 页面能力冻结；实现、浏览器、响应式、键盘、错误/空态、权限/租户证据仍未授权或 UNVERIFIED，等待 Checkpoint B 复审 |
