...
| TASK-030 | P0 | W6-C：物件索引 /properties List Report 参考迁移 | In Progress | TASK-029 | [TASK-030](docs/tasks/TASK-030.md) | Checkpoint C 有限实现和独立只读审查完成，无 P0/P1 阻塞；area 事实链、区域术语、费用 null/零值、listing price 非正数和 active/archived lifecycle 已落实；D-Lite 因本地 3002 绑定 EPERM 为 UNVERIFIED，禁止全站组件重构，真实运行证据按批次回归 |
