...
| TASK-026 | P0 | 建立可复用非生产 QA 环境 | In Progress | TASK-025 | [TASK-026](docs/tasks/TASK-026.md) | 两个专用非生产租户及合法 Clerk 身份、同一对象类型至少7条合成记录、正常路径归档/恢复、跨租户隔离和脱敏交接证据 |
...
- TASK-026 is In Progress as a separate QA-environment preparation task. It may establish only two dedicated non-production tenants, legal Clerk identities using existing roles, synthetic data, normal-path archive/restore, and cross-tenant evidence. It must not modify product code, business models, production data, authentication rules, permissions, or tenant isolation. TASK-025 remains Blocked until TASK-026's environment gates pass.
