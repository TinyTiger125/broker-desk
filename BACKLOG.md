...
| TASK-026 | P0 | 建立可复用非生产 QA 环境 | Blocked | TASK-025 | [TASK-026](docs/tasks/TASK-026.md) | 已创建两个专用非生产 QA 租户并发送 Clerk 开发邀请；QA B 已绑定、QA A 待接受，尚未创建业务测试数据，等待两个可登录身份后继续 |
...
- TASK-026 is Blocked as a separate QA-environment preparation task. Two dedicated non-production QA tenants were created through the formal platform account flow and Clerk development invitations were sent; QA B is bound, QA A is awaiting invitation acceptance, and no business test data has been created. It may establish only two dedicated non-production tenants, legal Clerk identities using existing roles, synthetic data, normal-path archive/restore, and cross-tenant evidence. It must not modify product code, business models, production data, authentication rules, permissions, or tenant isolation. TASK-025 remains Blocked until TASK-026's environment gates pass.
