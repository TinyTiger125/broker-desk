...
| TASK-030 | P0 | W6-C：物件索引 /properties List Report 参考迁移 | In Progress | TASK-029 | [TASK-030](docs/tasks/TASK-030.md) | Checkpoint B 方向批准；先修正 memory/PostgreSQL properties 列表适配的真实 area、区域术语、费用 null/零值、listing price 非正数和 active/archived lifecycle；随后一次有限实现、一次独立审查，再进入 D-Lite；禁止全站组件重构，真实运行证据仍按批次回归 |
