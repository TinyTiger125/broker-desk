...
| TASK-031 | P0 | W7-A：物件创建/编辑 Responsive Form 参考迁移 | Done | TASK-030 | [TASK-031](docs/tasks/TASK-031.md) | 页面结构迁移完成：共享字段/分组/服务端校验、保存/取消、错误结构和 PostgreSQL 租户作用域适配完成；独立代码审查无 P0/P1。1440/768/390、横向溢出、Kotoeri、Tab/Enter/焦点、返回上下文、PostgreSQL真实读写、权限/租户和完整无障碍进入全产品批次回归；IME仅有代码机制证据。下一项建议 W7-B 仅先审计 `/clients/new` 与 `/clients/[id]/edit`，保留客户自身业务语义 |
| TASK-032 | P0 | W7-B：客户创建/编辑 Responsive Form 参考迁移 | Done | TASK-031 | [TASK-032](docs/tasks/TASK-032.md) | 页面结构迁移完成：五组客户 Responsive Form、共享校验、单一保存/取消、错误结构、快速创建兼容和创建成功 `client_created` 三语反馈映射已收口；静态检查和独立只读审查通过。D-Lite 因本地 3002 绑定 `listen EPERM` 为 `UNVERIFIED`；1440/768/390、横溢出、真实 PostgreSQL、权限/租户、键盘/IME、焦点、返回和完整无障碍进入批次回归，不扩展 Object Page |
| TASK-033 | P0 | W7-C：主体创建/编辑 Responsive Form 参考迁移 | Done | TASK-032 | [TASK-033](docs/tasks/TASK-033.md) | 页面结构迁移完成：/parties/new 冻结为零表单 System State；编辑仅维护姓名、联系方式、显式类型和单一角色，notes 跨日中韩按类别规范化并保留状态/备注/未知行；严格 returnTo 白名单和共享事实安静说明；专项检查与独立只读审查通过；D-Lite 因 listen EPERM 为 UNVERIFIED，运行项进入批次回归 |
| TASK-034 | P0 | W11-CORE：全产品 Layout System 核心对象与工作流结构收口 | Done | TASK-033 | [TASK-034](docs/tasks/TASK-034.md) | 两批共八页完成页面结构收口：第一批基线 b9599c0；第二批 /cases/new RF、/workspace 唯一 Workspace Selector、/board 真实阶段 Kanban 响应式修正、/relationship-tree 显式关系 Explorer；专项脚本、typecheck、lint、build、workflow rules、diff check 和独立审查通过；真实运行项进入统一批次回归 |
