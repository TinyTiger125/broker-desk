...
| TASK-026 | P0 | 建立可复用非生产 QA 环境 | Blocked | TASK-025 | [TASK-026](docs/tasks/TASK-026.md) | 已确认正式 Clerk 开发登录和平台账户管理入口；阻塞于尚未确认两个专用非生产租户、合法 Clerk 测试身份及其所有权，尚未创建任何租户、身份或测试数据 |
...
- TASK-026 is Blocked as a separate QA-environment preparation task. The formal Clerk development login and platform account-management entry are reachable, but two dedicated non-production tenants, legal Clerk test identities, and ownership are not confirmed; no tenant, identity, or synthetic data has been created. It may establish only two dedicated non-production tenants, legal Clerk identities using existing roles, synthetic data, normal-path archive/restore, and cross-tenant evidence. It must not modify product code, business models, production data, authentication rules, permissions, or tenant isolation. TASK-025 remains Blocked until TASK-026's environment gates pass.
